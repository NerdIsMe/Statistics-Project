...
Plot_Year <- function(year)
{
  ...
  for (i in 3:8) # plot each column
  {
    ...
    #print(data_col)
    boxplot(data_col/divide[i-2], main = paste0( "民國" , year, "年", graph_name[i-2]), xlab = x_labs[i - 2], horizontal = T, col = "lightblue")
    ...
    if (length(index) != 0)
    {
      ...
      # Histogram
      hist(data_col/divide[i-2], main = paste("民國", year, "年 ", graph_name[i-2]), xlab = x_labs[i - 2], col = brewer.pal(n = 6, name = "RdBu"))
  ...
  for (i in c(1:6))
  {
    for (j in c(1:6))
    {
      ...
      else
      {
        ...
        plot(X,Y,xlab=graph_name[i],ylab=graph_name[j],main=paste("scatter diagram of",graph_name[i],"and",graph_name[j],"in",year),pch=19,col="blue",cex=0.5)
