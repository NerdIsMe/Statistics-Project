...
Scatter_Diagram <- function(year)
{
  ...
  #data
  data_one_year = data[data[, "Year"] == year,]
  items <- c("公車行駛段次", "公車行駛里程", "公車載客人次", "公車每段平均載客", "客運收入", "公車 每車公里營收")
  
  for(i in 3:7)
  {
    data_x = as.numeric(unlist(data_one_year[, i], use.names = F))
    for(j in ((i+1):8))
    {
      data_y = as.numeric(unlist(data_one_year[, j], use.names = F))
      plot(data_one_year[])
    }
  }
}
